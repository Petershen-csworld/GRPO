...
class Showo2Qwen2_5(ModelMixin, ConfigMixin):
    ...
    def __init__(
        ...
        num_params = sum(p.numel() for p in self.diffusion_head_a.parameters() if p.requires_grad)
        print(f"Number of parameters in diffusion_head_a: {num_params}")
    ...
    @torch.no_grad()
    def mmu_batch_generate(
        self,
        input_embeds=None,          # [batch, seq_len, dim]
        ...
        for _ in range(max_new_tokens):
            ...
            attention_mask_a = torch.hstack([
                attention_mask,
                torch.zeros((L, 1), device=device) + torch.finfo(logits.dtype).min,
            ])
            attention_mask_b = torch.vstack([
                attention_mask_a,
                torch.hstack([attention_mask[-1, :], torch.tensor([0], device=device)]).unsqueeze(0),
            ])
            ...
            # 对已完成的样本强制输出 eos_token
            if eos_token is not None:
                idx_next[finished] = eos_token

            # 保存到 result
            for i in range(batch_size):
                result[i].append(idx_next[i, 0].item())

            # 更新 finished 掩码
            if eos_token is not None:
                just_finished = (idx_next[:, 0] == eos_token)
                finished |= just_finished

            # 将 idx_next 转换成 embeddings，拼接到 input_embeds
