...
def _load_state_dict_into_model(
    ...
    def load(module: torch.nn.Module, prefix: str = "", assign_to_params_buffers: bool = False):
        ...
        params_to_gather = [p for p in module.parameters(recurse=False)]
        if any(getattr(p, "ds_status", None) is not None for p in params_to_gather):
    ...
    def from_pretrained(cls, pretrained_model_name_or_path: Optional[Union[str, os.PathLike]], **kwargs):
        ...
        if from_flax:
            ...
            if low_cpu_mem_usage:
                ...
                # WE CAN'T DO THIS with GRPOTrainer since it is wrapped in in deepspeed zero environment
                model = cls.from_config(config, **unused_kwargs)
        ...
        def _find_mismatched_keys(
            ...
            if ignore_mismatched_sizes:
                if is_deepspeed_zero3_enabled():
                    shapes = {k: v.ds_shape if hasattr(v, "ds_shape") else v.shape for k, v in model.named_parameters()}
                else:
                ...
                for checkpoint_key in loaded_keys:
                    ...
                    if (
                        ...
                        and model_key in shapes
                        and state_dict[checkpoint_key].shape != shapes[model_key]
                    ):
                        mismatched_keys.append(
                            (checkpoint_key, state_dict[checkpoint_key].shape, shapes[model_key])
                        )
